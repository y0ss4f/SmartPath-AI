          ...
          {completed.length > 0 && (
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-gray-50 rounded-xl p-3 text-center">
                ...
                <p className="text-lg font-bold text-gray-700 mt-0.5">{avgInitial.toFixed(1)}<span className="text-xs text-gray-400">/5</span></p>
              </div>
              <div className="bg-gray-50 rounded-xl p-3 text-center">
                <p className="text-[10px] uppercase tracking-wider text-gray-400 font-medium">Score final</p>
                <p className="text-lg font-bold text-gray-700 mt-0.5">{avgFinal.toFixed(1)}<span className="text-xs text-gray-400">/5</span></p>
              </div>
              ...
              <div key={quiz.id} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-xl">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-xs text-gray-700 truncate">
                    {quiz.subject || 'Sans matière'}
                  </span>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {quiz.initial_score !== null && quiz.final_score !== null ? (
                    <>
                      <span className="text-xs text-gray-500">{quiz.initial_score}/5</span>
                      <span className="text-xs text-gray-300">→</span>
                      <span className="text-xs font-semibold text-gray-700">{quiz.final_score}/5</span>
