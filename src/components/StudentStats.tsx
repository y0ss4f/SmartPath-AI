...
import { TrendingUp, TrendingDown, Minus, ChevronDown, ChevronUp, BarChart3, BookOpen, Clock } from 'lucide-react'
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            {quizzes.slice(0, 5).map((quiz) => {
              const totalSeconds = quiz.time_spent_per_question?.reduce((sum, t) => sum + t.seconds, 0) || 0;
              ...
              return (
                <div key={quiz.id} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {/* Time Spent */}
                    {timeString && (
                      <div className="flex items-center gap-1 ml-1" title="Temps total (Éval. + Rếéval.)">
                        <Clock className="w-3 h-3 text-gray-400" />
                        <span className="text-[10px] text-gray-500 font-medium">{timeString}</span>
                      </div>
                    )}

                    {/* Perceived difficulty */}
                    {quiz.perceived_difficulty && PERCEIVED_LABELS[quiz.perceived_difficulty] && (
                      <span className="text-xs ml-1" title={PERCEIVED_LABELS[quiz.perceived_difficulty].label}>
